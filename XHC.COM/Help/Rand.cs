...
namespace XHC.COM
{
    ...
    public class YZMHelper
    {
        ...
        public string base64 { get { return Convert.ToBase64String(stream.ToArray()); } }
        public MemoryStream stream { get { MemoryStream ms = new MemoryStream(); this.Image.Save(ms, ImageFormat.Jpeg); return ms; } }
        #endregion
        ...
        #region 构造函数
        public YZMHelper(RandImg img = RandImg.Int, int len = 4)
        {
            ...
            this.letterCount = len;
            switch (img)
            {
                case RandImg.Int:
                    this.text = Rand.Number(letterCount);
                    break;
                case RandImg.Str:
                    this.text = Rand.Str(letterCount);
                    break;
                case RandImg.Char:
                    this.text = Rand.Str_char(letterCount);
                    break;
                default:
                    this.text = Rand.Number(letterCount);
                    break;
